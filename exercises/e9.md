...
Otherwise, return the solution, which is guaranteed to have the expected cost at most $\Theta\left(\sqrt{\frac{\ln n}{T}}\right)$ more than $OPT$.
...
Let $a^\*$ be the payoff-maximizing action and 

$$
OPT = \sum_{t=1}^T \pi^t(a^\*).
$$
...
\frac{1}{T} \left[
    ...
    - \mathbb E_{\mathbf s \sim \sigma^t} [c_i(s_i^\*, \mathbf s_{-i})]
\right]
\le R_i,
$$
...
_[unintentionally left blank :(]_
