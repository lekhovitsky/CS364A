...
Recall the game from the lecture with two players $\mathcal N = \{1, 2\}$, three locations $\mathcal L = \{1, 2, 3\}$ and two markets $\mathcal M = \{1, 2\}$. 

* Player 1 can sell from locations 1 and 2, player 2 &mdash; from 2 and 3;
* Market 1 can buy from locations 1 and 2, market 2 &mdash; from 2 and 3;
* Selling from locations 1 and 3 costs 1, from location 2 &mdash; 2;
* Values of both markets are 3.

The optimal equilibrium in this game is $a^* = (1, 3)$. Both players sell to corresponding markets at maximum price, each makes 2 units of cash, and the social surplus of such allocation is 4.

The suboptimal equilibrium is $a = (2, 2)$. In this case, both players still sell for the maximum price but also for higher cost, so each makes 1 unit of cash, and the social surplus is 2 (which is exactly half of the optimal). Deviating to other location makes no sense because player would be forced to lower the price, compensating for the decreased cost.
