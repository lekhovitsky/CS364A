...
which implies 
$$
\hat x^\top A \hat y \ge \max_x \min_y x^\top A y. \tag{1}
$$
...
meaning 
$$
\hat x^\top A \hat y \le \min_y \max_x x^\top A y. \tag{2}
$$
...
The reverse implication simply follows from the fact that NRD strategies that were constructed to converge to a minimax pair also converge to a MNE. Let $\hat x$ and $\hat y$ be the time-averaged mixed strategies of NRD after $T$ iterations. In the lecture we've shown that
$$
