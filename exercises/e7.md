...
From the lecture we know that $\pi_i(a) = V(a) - V(a_{-i})$, where $V(a) = \sum_{j \in M} v_{j} - d_j(a)$.
...
Let $a \in \mathcal A$ be an equilibrium action profile, and $a^* \in \mathcal A$  be an optimal one.
Then we have
...
$$
C(a) := \sum_{i \in \mathcal N} c_i(a) \le (1 + \epsilon) \sum_{i \in \mathcal N} c_i(a_i^*, a_{-i}). \ \ \ \ (*)
$$
